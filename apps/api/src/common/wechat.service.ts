...
export class WeChatService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.devMode = this.configService.get<string>('WECHAT_DEV_MODE') === 'true';
    ...
    if (this.devMode) {
      this.logger.warn(
        'WeChat service running in development mode - authentication will be mocked',
      );
    }
  }
  ...
  async code2Session(code: string): Promise<WeChatSession> {
    // 开发模式：返回模拟数据
    if (this.devMode) {
      this.logger.log('Development mode: returning mock WeChat session data');
      return {
    ...
    try {
      ...
      if (data.errcode) {
        this.logger.error(
          `WeChat code2session error: ${data.errcode} - ${data.errmsg}`,
        );
      ...
      this.logger.log(
        `Successfully got WeChat session for openid: ${data.openid.substring(0, 8)}...`,
      );
      return data;
    } catch (error) {
